...
void dfs(Huff_node *raiz, struct huff_hash *ht, int flag, unsigned char shift_bit, int level)
{
	...
	else
	{
		if(flag == 1)
		{
			/*return 1 + */dfs(raiz->down_left, ht, 0, shift_bit + 0, level + 1);
			/*return 1 + */dfs(raiz->down_right, ht, 0, shift_bit + 1, level + 1);
		}
		else
		{
			/*return 1 + */dfs(raiz->down_left, ht, 0, (shift_bit<<1), level + 1);
			/*return 1 + */dfs(raiz->down_right, ht, 0, (shift_bit<<1) + 1, level + 1);
		}
	}
}
