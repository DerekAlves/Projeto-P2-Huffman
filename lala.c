...
Node_index *add_element(Node_index *local, void *item, unsigned char shift_bit, int level)
{
	local->item = item;
...
void put(Hash_Table *ht, void *item, unsigned char shift_bit, int level)
{
	int index;
	int item_aux = item;
...
void dfs(Element_queue *raiz, Hash_Table *ht, int flag, unsigned char shift_bit, int level)
{
	//printf("%c, %d\n", raiz->item, shift_bit);
